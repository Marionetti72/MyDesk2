"""
TRADING SYSTEM - BACKTEST E GENERAZIONE SEGNALI (FIXED)
================================================
...
print("=" * 100)
print("TRADING SYSTEM - BACKTEST E GENERAZIONE SEGNALI (FIXED)")
print("=" * 100)
...
# ============================================================================
# FASE 4: GENERAZIONE SEGNALI (FIXED)
# ============================================================================
...
def generate_signals(indicators, rsi_oversold=30, rsi_overbought=70, macd_threshold=0):
    ...
    FIXED: Corretti errori logica booleana Pandas
    """
    ...
    buy_condition = (
        ...
        (signals['MACD_Hist'] > macd_threshold) &
        (signals['SMA50'] > signals['SMA200'])
    )
    signals['BUY'] = buy_condition.astype(int)
    ...
    signals['SELL'] = sell_condition.astype(int)
    
    # Consolidamento: se BUY e SELL contemporaneamente, no segnale
    conflicting = (buy_condition) & (sell_condition)
    signals.loc[conflicting, 'BUY'] = 0
    signals.loc[conflicting, 'SELL'] = 0
...
for titolo, indicators in indicators_all.items():
    try:
        ...
        print(f"⚠ Errore generazione segnali {titolo}: {e}")
...
print(f"✓ Totale segnali BUY: {int(total_buy_signals)}")
print(f"✓ Totale segnali SELL: {int(total_sell_signals)}")
